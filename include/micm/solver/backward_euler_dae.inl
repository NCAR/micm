...
enum class MicmBackwardEulerDAEErrc
{
  FailedToConverge = 1,
  ConstraintsNotSatisfied = 2
};

namespace std
{
  template<>
  struct is_error_condition_enum<MicmBackwardEulerDAEErrc> : true_type
  {
  };
}  // namespace std
...
namespace
{
  class BackwardEulerDAEErrorCategory : public std::error_category
  {
   public:
    const char* name() const noexcept override
    {
      return "MICM BackwardEulerDAE";
    }
    std::string message(int ev) const override
    {
      switch (static_cast<MicmBackwardEulerDAEErrc>(ev))
      {
        case MicmBackwardEulerDAEErrc::FailedToConverge: return "Failed to converge";
        case MicmBackwardEulerDAEErrc::ConstraintsNotSatisfied: return "Algebraic constraints not satisfied";
        default: return "Unknown error";
      }
    }
  };

  const BackwardEulerDAEErrorCategory backwardEulerDAEErrorCategory{};
}  // namespace

inline std::error_code make_error_code(MicmBackwardEulerDAEErrc e)
{
  return { static_cast<int>(e), backwardEulerDAEErrorCategory };
}

namespace micm
{
  template<class RatesPolicy, class LinearSolverPolicy>
  inline SolverResult AbstractBackwardEulerDAE<RatesPolicy, LinearSolverPolicy>::Solve(
      double time_step,
      auto& state,
      const BackwardEulerDAESolverParameters& parameters) const
  {
    // A fully implicit euler implementation with algebraic constraints is given by:
    // y_{n+1} = y_n + H * f(t_{n+1}, y_{n+1})
    // 0 = g(t_{n+1}, y_{n+1})  (algebraic constraints)
    // This is a root finding problem because you need to know y_{n+1} to compute f(t_{n+1}, y_{n+1})
    // and satisfy the constraints g(t_{n+1}, y_{n+1}) = 0
    // You need to solve the equation y_{n+1} - y_n - H f(t_{n+1}, y_{n+1}) = 0
    // while ensuring g(t_{n+1}, y_{n+1}) = 0
    ...
    auto derived_class_temporary_variables =
        static_cast<BackwardEulerDAETemporaryVariables<DenseMatrixPolicy>*>(state.temporary_variables_.get());
    ...
    auto& constraint_values = derived_class_temporary_variables->constraint_values_;
    ...
    while (t < time_step)
    {
      ...
      do
      {
        ...
        // Evaluate algebraic constraints after updating the solution
        EvaluateConstraints(t + H, state, parameters, constraint_values);
        ...
        // Also check if algebraic constraints are satisfied
        if (converged && !AreConstraintsSatisfied(constraint_values))
        {
          converged = false;
        }
  ...
  inline void AbstractBackwardEulerDAE<RatesPolicy, LinearSolverPolicy>::EvaluateConstraints(
      double time,
      const auto& state,
      const BackwardEulerDAESolverParameters& parameters,
      std::vector<double>& constraint_values) const
  {
    constraint_values.resize(parameters.algebraic_constraints_.size());
    for (std::size_t i = 0; i < parameters.algebraic_constraints_.size(); ++i)
    {
      // Convert the matrix to a vector format for the constraint function
      auto variables_vector = state.variables_.AsVector();
      constraint_values[i] = parameters.algebraic_constraints_[i](time, variables_vector, state.rate_constants_);
    }
  }

  template<class RatesPolicy, class LinearSolverPolicy>
  inline bool AbstractBackwardEulerDAE<RatesPolicy, LinearSolverPolicy>::AreConstraintsSatisfied(
  ...
  template<class RatesPolicy, class LinearSolverPolicy>
  template<class DenseMatrixPolicy>
  inline bool AbstractBackwardEulerDAE<RatesPolicy, LinearSolverPolicy>::IsConverged(
      const BackwardEulerDAESolverParameters& parameters,
