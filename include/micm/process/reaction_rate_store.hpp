  ...
  struct ReactionRateConstantStore
  {
    ...
    static void EvaluateCpuRateConstants(const ReactionRateConstantStore& store, StatePolicy& state)
    {
      ...
      if constexpr (VectorizableDense<DenseMatrixPolicy>)
      {
        ...
        for (Index i_group = 0; i_group < state.rate_constants_.NumberOfGroups(); ++i_group)
        {
          const Index rc_base = i_group * state.rate_constants_.GroupSize();
          const Index n_local = std::min(L, state.rate_constants_.NumRows() - i_group * L);

          for (Index i_cell = 0; i_cell < n_local; ++i_cell)
          {
      ...
      {
        for (Index i_cell = 0; i_cell < state.rate_constants_.NumRows(); ++i_cell)
        {
    ...
    static void CpuCalculateRateConstants(const ReactionRateConstantStore& store, StatePolicy& state)
    {
      ...
      auto calc = DenseMatrixPolicy::Function(
          [&store, &cond = state.conditions_](auto&& rc, auto&& cp)
          {
            ...
            {
              rc.ForEachRow(
                  [&mult](Real& v, const Conditions& c) { v *= mult.evaluate_(c); },
