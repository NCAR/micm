  ...
  class KokkosProcessSet : public ProcessSet<DenseMatrixPolicy, SparseMatrixPolicy>
  {
   public:
    ...
    void UpdateDevStruct()
    {
      ...
      for (std::size_t i = 0; i < this->number_of_reactants_.size(); ++i) h_number_of_reactants(i) = this->number_of_reactants_[i];
      ...
      for (std::size_t i = 0; i < this->reactant_ids_.size(); ++i) h_reactant_ids(i) = this->reactant_ids_[i];
      ...
      for (std::size_t i = 0; i < this->number_of_products_.size(); ++i) h_number_of_products(i) = this->number_of_products_[i];
      ...
      for (std::size_t i = 0; i < this->product_ids_.size(); ++i) h_product_ids(i) = this->product_ids_[i];
      ...
      for (std::size_t i = 0; i < this->yields_.size(); ++i) h_yields(i) = this->yields_[i];
    ...
    void SetJacobianFlatIds(const SparseMatrixPolicy& matrix)
    {
      ...
      for (std::size_t i = 0; i < this->jacobian_process_info_.size(); ++i)
      {
        ...
        h_jacobian_process_info(i).number_of_dependent_reactants_ = this->jacobian_process_info_[i].number_of_dependent_reactants_;
      ...
      devstruct_.jacobian_reactant_ids_ = Kokkos::View<std::size_t*>("jacobian_reactant_ids", this->jacobian_reactant_ids_.size());
      auto h_jacobian_reactant_ids = Kokkos::create_mirror_view(devstruct_.jacobian_reactant_ids_);
      for (std::size_t i = 0; i < this->jacobian_reactant_ids_.size(); ++i) h_jacobian_reactant_ids(i) = this->jacobian_reactant_ids_[i];
      Kokkos::deep_copy(devstruct_.jacobian_reactant_ids_, h_jacobian_reactant_ids);

      devstruct_.jacobian_product_ids_ = Kokkos::View<std::size_t*>("jacobian_product_ids", this->jacobian_product_ids_.size());
      auto h_jacobian_product_ids = Kokkos::create_mirror_view(devstruct_.jacobian_product_ids_);
      for (std::size_t i = 0; i < this->jacobian_product_ids_.size(); ++i) h_jacobian_product_ids(i) = this->jacobian_product_ids_[i];
      ...
      for (std::size_t i = 0; i < this->jacobian_yields_.size(); ++i) h_jacobian_yields(i) = this->jacobian_yields_[i];
      ...
      for (std::size_t i = 0; i < this->jacobian_flat_ids_.size(); ++i) h_jacobian_flat_ids(i) = this->jacobian_flat_ids_[i];
    ...
    void SubtractJacobianTerms(const auto& state, const DenseMatrixPolicy& state_variables, SparseMatrixPolicy& jacobian) const
    {
      auto d_rate_constants = state.rate_constants_.GetView();
      auto d_state_variables = state_variables.GetView();
