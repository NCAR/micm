     ...
     public:
      ...
      Proxy &operator=(const std::vector<T> &other)
      {
        if (other.size() < y_dim_)
        {
          ...
          throw micm::MicmCodedError<MicmMatrixErrc>(MicmMatrixErrc::RowSizeMismatch, micm::MicmSeverity::Error, msg);
        }
              ...
              [&]() -> std::vector<T>
              {
                ...
                for (auto &other_row : other)
                {
                  if (other_row.size() != y_dim)
                  {
                    ...
                    throw micm::MicmCodedError<MicmMatrixErrc>(MicmMatrixErrc::InvalidVector, micm::MicmSeverity::Error, msg);
                  }
      ...
      auto result = [func = std::forward<Func>(func), num_cols = std::move(num_cols)](auto&&... invoked_args) mutable {
        ...
        ([&](auto& arg) {
          ...
          if constexpr (VectorLike<ArgType>) {
            ...
            {
              throw micm::MicmCodedError<MicmMatrixErrc>(MicmMatrixErrc::InvalidVector, micm::MicmSeverity::Error,
