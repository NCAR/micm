...
- **Related:** NCAR/musica#956 Case 2; `docs/superpowers/notes/2026-07-26-musica956-knife-edge-repro.md`; ODE repo `paper_dae/` §4, `update_dae/update_dae.tex`
...
That change is correct and it resolves the musica#956 Case-2 knife edge. It does
**not** globalize the Newton iteration. `InitializeConstraints` still applies a
full, undamped step every update (`rosenbrock.inl` step 9, `apply_update(Y, delta)`).
...
Use existing temporaries; add no new allocation:
