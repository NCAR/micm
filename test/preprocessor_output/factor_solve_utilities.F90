...
! This tag was created on undefined by undefined and is marked as not buggy
  ...
  integer, parameter, public  :: number_sparse_factor_elements      = 23
...
subroutine backsolve_L_y_eq_b(LU,b,y)
  ...
  real(r8), intent(in) :: LU(ncell,number_sparse_factor_elements)
  real(r8), intent(in) :: b(ncell,number_of_species)
  real(r8), intent(out) :: y(ncell,number_of_species)
...
subroutine backsolve_U_x_eq_y(LU,y,x)
  ...
  real(r8), intent(in) :: LU(ncell,number_sparse_factor_elements)
  real(r8), intent(in) :: y(ncell,number_of_species)
  real(r8), intent(out) :: x(ncell,number_of_species)
...
subroutine factor(LU)
  ...
  real(r8), intent(inout) :: LU(ncell,number_sparse_factor_elements)
...
subroutine solve(LU,x,b)
  ...
  real(r8), intent(in) :: LU(ncell,number_sparse_factor_elements), &
                          b(ncell,number_of_species) ! solve LU * x = b
  real(r8), intent(out) :: x(ncell,number_of_species)

  ! Local variables
  real(r8) :: y(ncell,number_of_species)

  !$acc enter data create(y) async(STREAM2) wait(STREAM3)

  call backsolve_L_y_eq_b(LU, b, y)
  call backsolve_U_x_eq_y(LU, y, x)

  !$acc exit data delete(y) async(STREAM2)
