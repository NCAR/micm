...
std::vector<double> call_fortran_dforce_dy_times_vector(std::vector<double>& dforce_dy, std::vector<double>& vector){
  ...
  CFI_establish((CFI_cdesc_t *)&product, NULL,
  ...
  CFI_establish((CFI_cdesc_t *)&f_vector, vector.data(),
                      CFI_attribute_other,
                      CFI_type_double, vector.size() * sizeof(double), (CFI_rank_t)1, vector_extent);

  dforce_dy_times_vector(
