...
void TestJacobian(SolverPolicy& solver)
{
  ...
  static const std::vector<std::pair<micm::Index, micm::Index>> hardcoded_positions = {
  ...
  for (micm::Index i{}; i < 3; ++i)
  {
    ...
    std::vector<micm::Real> fixed_jacobian = fixed_solver.DforceDy(rate_constants, variables, number_density_air);

    EXPECT_EQ(fixed_jacobian.size(), hardcoded_positions.size());

    for (micm::Index j{}; j < fixed_jacobian.size(); ++j)
    {
      auto [hc_row, hc_col] = hardcoded_positions[j];
      micm::Index modern_row = state.variable_map_[fixed_solver.SpeciesNames()[hc_row]];
      micm::Index modern_col = state.variable_map_[fixed_solver.SpeciesNames()[hc_col]];
