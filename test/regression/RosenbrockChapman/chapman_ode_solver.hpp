...
namespace micm
{
  ...
  inline State<> ChapmanODESolver::GetState(const std::size_t NUMBER_OF_GRID_CELLS = 1) const
  {
    auto state_parameters = micm::StateParameters{
      ...
      .variable_names_ = SpeciesNames(),
      .custom_rate_parameter_labels_ = PhotolysisNames(),
    };
  ...
  inline ChapmanODESolver::SolverResult ChapmanODESolver::Solve(double time_start, double time_end, State<>& state) noexcept
  {
    ...
    {
      if (stats_.number_of_steps_ > parameters_.max_number_of_steps_)
      {
      ...
      while (!accepted)
      {
        ...
        auto ode_jacobian = LinFactor(h, parameters_.gamma_[0], Y, NUMBER_DENSITY_AIR, rate_constants);
        ...
        for (uint64_t stage = 0; stage < parameters_.stages_; ++stage)
        {
          ...
          else
          {
            ...
            if (parameters_.new_function_evaluation_[stage])
            {
              ...
              for (uint64_t j = 0; j < stage; ++j)
              {
                ...
                for (uint64_t idx = 0; idx < NewY.size(); ++idx)
                {
                  NewY[idx] += a * K[j][idx];
                }
              }
              forcing = Force(rate_constants, NewY, NUMBER_DENSITY_AIR);
            }
            K[stage] = forcing;
            for (uint64_t j = 0; j < stage; ++j)
            {
          ...
          K[stage] = LinSolve(K[stage], ode_jacobian);
        }
        ...
        for (uint64_t stage = 0; stage < parameters_.stages_; ++stage)
        {
          for (uint64_t idx = 0; idx < NewY.size(); ++idx)
          {
            NewY[idx] += parameters_.m_[stage] * K[stage][idx];
          }
        }
        ...
        auto error = ErrorNorm(Y, NewY, Yerror);
