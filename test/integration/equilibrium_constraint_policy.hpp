...
// Equilibrium-constraint integration tests, shared by the CPU and Kokkos drivers so the two
// cannot drift apart.
//
// Note on the device syncs below: `state.custom_rate_parameters_.CopyToHost()` AFTER
// UpdateStateParameters() is deliberate and necessary here. UpdateStateParameters() evaluates
// the Van't Hoff equilibrium constant on the device and writes it into
// custom_rate_parameters_, and these tests assert on that value, so it has to be downloaded.
// That is the opposite direction from linear_constraint_policy.hpp, where the rate constants
// are host inputs that must be uploaded. CopyToHost/CopyToDevice are no-ops on the CPU matrix
// types, so both backends run the identical call sequence.
//
// Each test takes a factory rather than a builder because every body constructs its own
// RosenbrockSolverParameters (some adjust h_start_ for float builds) and a builder is consumed
// by Build(). The `using namespace micm;` is function-local on purpose: it keeps the bodies
// identical to the originals without leaking the directive into every translation unit that
...
inline micm::Real ComputeEquilibriumConstant(micm::Real K_HLC_ref, micm::Real delta_H, micm::Real T)
{
  return K_HLC_ref * std::exp((delta_H / micm::constants::GAS_CONSTANT) * (1.0 / T - 1.0 / 298.15));
}
...
void TestSetConstraintsAPIWorks(BuilderFactory make_builder)
{
  using namespace micm;
  using BuilderPolicy = std::invoke_result_t<BuilderFactory&, const micm::RosenbrockSolverParameters&>;
...
template<class BuilderFactory>
void TestSetConstraintsAPIMultipleConstraints(BuilderFactory make_builder)
{
  using namespace micm;
  using BuilderPolicy = std::invoke_result_t<BuilderFactory&, const micm::RosenbrockSolverParameters&>;
  using DenseMatrix = typename BuilderPolicy::DenseMatrixPolicyType;
...
template<class BuilderFactory>
void TestDAESolveWithConstraint(BuilderFactory make_builder)
{
  using namespace micm;
  using BuilderPolicy = std::invoke_result_t<BuilderFactory&, const micm::RosenbrockSolverParameters&>;
  using DenseMatrix = typename BuilderPolicy::DenseMatrixPolicyType;
...
template<class BuilderFactory>
void TestDAESolveWithConstraintAndReorderState(BuilderFactory make_builder)
{
  using namespace micm;
  using BuilderPolicy = std::invoke_result_t<BuilderFactory&, const micm::RosenbrockSolverParameters&>;
  using DenseMatrix = typename BuilderPolicy::DenseMatrixPolicyType;
...
template<class BuilderFactory>
void TestDAESolveWithTwoCoupledConstraints(BuilderFactory make_builder)
{
  using namespace micm;
  using BuilderPolicy = std::invoke_result_t<BuilderFactory&, const micm::RosenbrockSolverParameters&>;
  using DenseMatrix = typename BuilderPolicy::DenseMatrixPolicyType;
...
template<class BuilderFactory>
void TestDAESolveWithNonUnitStoichiometry(BuilderFactory make_builder)
{
  using namespace micm;
  using BuilderPolicy = std::invoke_result_t<BuilderFactory&, const micm::RosenbrockSolverParameters&>;
  using DenseMatrix = typename BuilderPolicy::DenseMatrixPolicyType;
