...
// CopyToDevice()/CopyToHost() calls are no-ops on the CPU matrix types
// (util/matrix.hpp, util/vector_matrix.hpp, util/padded_vector.hpp), so both backends run
...
void TestAlgebraicVariableStaysNonNegative(BuilderPolicy builder)
{
  ...
  while (advanced < dt)
  {
    ...
    ASSERT_EQ(result.state_, micm::SolverState::Converged) << "Solver did not converge at t=" << advanced;
...
template<class BuilderPolicy>
void TestEquilibriumPlusConservation(BuilderPolicy builder)
{
  using DenseMatrix = typename BuilderPolicy::DenseMatrixPolicyType;
  using StdSparseMatrix = typename BuilderPolicy::SparseMatrixPolicyType;
  ...
  micm::Phase gas_phase{ "gas", std::vector<micm::PhaseSpecies>{ A_gas, A_aq, P } };
...
///
/// Takes a factory rather than a builder because each parameter set needs its own builder.
template<class BuilderFactory>
void TestAllRosenbrockOrdersConstrained(BuilderFactory make_builder)
{
  ...
  const std::vector<std::pair<std::string, micm::RosenbrockSolverParameters>> parameter_sets = {
    { "three-stage", micm::RosenbrockSolverParameters::ThreeStageRosenbrockParameters() },
    { "four-stage", micm::RosenbrockSolverParameters::FourStageRosenbrockParameters() },
    { "four-stage-dae", micm::RosenbrockSolverParameters::FourStageDifferentialAlgebraicRosenbrockParameters() },
    { "six-stage-dae", micm::RosenbrockSolverParameters::SixStageDifferentialAlgebraicRosenbrockParameters() },
  };
  ...
  for (const auto& [name, options] : parameter_sets)
  {
    ...
    micm::Process rxn = micm::ChemicalReactionBuilder()
                            .SetReactants({ A })
                            .SetProducts({ { B, 1 } })
                            .SetRateConstant(micm::ArrheniusRateConstantParameters{ .A_ = 1.0e4, .B_ = 0, .C_ = 0 })
                            .SetPhase(gas_phase)
                            .Build();
    ...
    while (advanced < dt)
    {
      ...
      ASSERT_EQ(result.state_, micm::SolverState::Converged)
          << "Solver did not converge for " << name << " at t=" << advanced;
      advanced += result.stats_.final_time_;
      state.variables_.CopyToHost();
