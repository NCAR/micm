...
{
  params.k0_ = 2.0e-22 * micm::constants::AVOGADRO_CONSTANT * 1.0e-6 * std::exp(static_cast<micm::Real>(params.n_));
  micm::Real air_ref = 2.45e19 / micm::constants::AVOGADRO_CONSTANT * 1.0e6;
  micm::Real a = params.k0_ * air_ref;
  micm::Real b = 0.43 * std::pow(293.0 / 298.0, -8.0);
  micm::Real A_val = a / (1.0 + a / b) * std::pow(0.41, 1.0 / (1.0 + std::pow(std::log10(a / b), 2.0)));
...
TEST(BranchedRateConstant, CalculateAlkoxyBranchWithAllArguments)
{
  ...
  micm::Real k = micm::CalculateBranched(params, conditions.temperature_, conditions.air_density_);

  micm::Real air_dens_n_cm3 = 42.2 * micm::constants::AVOGADRO_CONSTANT * 1.0e-6;
  micm::Real a = 2.0e-22 * std::exp(2) * 2.45e19;
  micm::Real b = 0.43 * std::pow((293.0 / 298.0), -8.0);
  micm::Real z = a / (1.0 + a / b) * std::pow(0.41, 1.0 / (1.0 + std::pow(std::log10(a / b), 2))) * (1.0 - 1.0e-3) / 1.0e-3;
  a = 2.0e-22 * std::exp(2) * air_dens_n_cm3;
  b = 0.43 * std::pow((temperature / 298.0), -8.0);
  micm::Real A = a / (1.0 + a / b) * std::pow(0.41, 1.0 / (1.0 + std::pow(std::log10(a / b), 2)));
  micm::Real expected = 1.2 * std::exp(-204.3 / temperature) * (z / (z + A));
...
TEST(BranchedRateConstant, CalculateNitrateBranchWithAllArguments)
{
  ...
  b = 0.43 * std::pow((temperature / 298.0), -8.0);
  micm::Real A = a / (1.0 + a / b) * std::pow(0.41, 1.0 / (1.0 + std::pow(std::log10(a / b), 2)));
  micm::Real expected = 1.2 * std::exp(-204.3 / temperature) * (A / (z + A));
  EXPECT_NEAR(k, expected, TOLERANCE * expected);
}
