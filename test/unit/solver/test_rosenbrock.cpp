...
// Robertson's problem with a first step spanning the whole interval is
// rejected several times in a row before the controller finds an acceptable H.
// The non-in-place LU path reuses the shifted Jacobian across those attempts,
// applying only the change in alpha; last_alpha must track the cumulative
// shift present in the matrix, not the per-attempt delta, or the third and
// later attempts of a step factor [(alpha_k + alpha_1) * M - J]. This test
// pins the standard (non-in-place) builder to the in-place builder, which
// rebuilds the Jacobian every attempt and cannot accumulate such an error.
TEST(RosenbrockSolver, RejectedStepAlphaAccumulation)
{
  auto A = micm::Species("A");
  auto B = micm::Species("B");
  auto C = micm::Species("C");
  ...
  micm::Process r1 = micm::ChemicalReactionBuilder()
                         .SetReactants({ A })
                         .SetProducts({ micm::StoichSpecies(B, 1) })
                         .SetRateConstant(micm::ArrheniusRateConstantParameters{ .A_ = 0.04 })
                         .SetPhase(gas_phase)
                         .Build();
  micm::Process r2 = micm::ChemicalReactionBuilder()
                         .SetReactants({ B, B })
                         .SetProducts({ micm::StoichSpecies(B, 1), micm::StoichSpecies(C, 1) })
                         .SetRateConstant(micm::ArrheniusRateConstantParameters{ .A_ = 3.0e7 })
                         .SetPhase(gas_phase)
                         .Build();
  micm::Process r3 = micm::ChemicalReactionBuilder()
                         .SetReactants({ B, C })
                         .SetProducts({ micm::StoichSpecies(A, 1), micm::StoichSpecies(C, 1) })
                         .SetRateConstant(micm::ArrheniusRateConstantParameters{ .A_ = 1.0e4 })
                         .SetPhase(gas_phase)
                         .Build();

  constexpr double time_step = 1000.0;
  auto options = micm::RosenbrockSolverParameters::ThreeStageRosenbrockParameters();
  options.h_start_ = time_step;  // absurd first step, so the first attempts reject in succession
  ...
  auto run = [&](auto builder) -> RunResult
  {
    auto solver = builder.SetSystem(micm::System(gas_phase))
    ...
    return { result.stats_,
             { state.variables_[0][state.variable_map_.at("A")],
               state.variables_[0][state.variable_map_.at("B")],
               state.variables_[0][state.variable_map_.at("C")] } };
  };

  auto standard = run(micm::CpuSolverBuilder<micm::RosenbrockSolverParameters>(options));
  auto in_place = run(micm::CpuSolverBuilderInPlace<micm::RosenbrockSolverParameters>(options));

  // The scenario must actually exercise repeated rejections of a single step
  // (number_of_steps_ counts attempts; rejections of the very first step are
  // not tallied in rejected_).
  EXPECT_GE(standard.stats.number_of_steps_ - standard.stats.accepted_, 2);

  // Both builders must walk the same step sequence and land on the same state.
  EXPECT_EQ(standard.stats.number_of_steps_, in_place.stats.number_of_steps_);
  EXPECT_EQ(standard.stats.accepted_, in_place.stats.accepted_);
  EXPECT_EQ(standard.stats.rejected_, in_place.stats.rejected_);
  for (std::size_t i = 0; i < standard.concentrations.size(); ++i)
  {
    const double scale = std::max(std::abs(standard.concentrations[i]), std::abs(in_place.concentrations[i]));
    const double diff = std::abs(standard.concentrations[i] - in_place.concentrations[i]);
    EXPECT_LT(diff, 1.0e-10 * std::max(scale, 1.0e-30));
  }
}
