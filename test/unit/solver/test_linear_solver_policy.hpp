...
void TestDenseMatrix()
{
  ...
  auto lu = micm::LuDecomposition<SparseMatrixPolicy>::GetLUMatrices(A, 0, false);
  ...
  solver.Solve(x, lower_matrix, upper_matrix);
...
void TestExtremeInitialValue(micm::Index number_of_blocks, micm::Real initial_value)
{
  ...
  auto lu = micm::LuDecomposition<SparseMatrixPolicy>::GetLUMatrices(A, initial_value, false);
  auto lower_matrix = std::move(lu.first);
  auto upper_matrix = std::move(lu.second);
...
void TestMarkowitzReordering()
{
  ...
  auto orig_LU_calc = micm::LuDecomposition<SparseMatrixPolicy>::Create(orig_jac);
  auto reordered_LU_calc = micm::LuDecomposition<SparseMatrixPolicy>::Create(reordered_jac);

  auto orig_LU = orig_LU_calc.GetLUMatrices(orig_jac, 0.0, false);
  auto reordered_LU = reordered_LU_calc.GetLUMatrices(reordered_jac, 0.0, false);
