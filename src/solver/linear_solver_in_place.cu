...
namespace micm::cuda
{
  ...
  LinearSolverInPlaceParam CopyConstData(LinearSolverInPlaceParam& hoststruct)
  {
    ...
    CHECK_CUDA_ERROR(
        ...
        "cudaMalloc");
    CHECK_CUDA_ERROR(
        cudaMallocAsync(&(devstruct.Lij_yj_), Lij_yj_bytes, micm::cuda::CudaStreamSingleton::GetInstance().GetCudaStream(0)),
        "cudaMalloc");
    CHECK_CUDA_ERROR(
        cudaMallocAsync(
        ...
        "cudaMalloc");
    CHECK_CUDA_ERROR(
        cudaMallocAsync(&(devstruct.Uij_xj_), Uij_xj_bytes, micm::cuda::CudaStreamSingleton::GetInstance().GetCudaStream(0)),
        "cudaMalloc");
