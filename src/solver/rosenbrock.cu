                                                ...
                                                const double alpha)
{
        ...
        if (tid < n_grids)
    {
        ...
        {
            size_t jacobian_index = jacobian_diagonal_elements[j];
                        ...
                        const double alpha)
    {
        ...
        cudaMalloc(&d_jacobian, sizeof(double)*jacobian_diagonal_elements.size());
        ...
        AlphaMinusJacobianKernel(sparseMatrix.n_grids,
                                d_jacobian,  
                                d_jacobian_diagonal_elements,
                                jacobian_diagonal_elements.size(),
                                alpha);
        cudaMemcpy(sparseMatrix.jacobian_, d_jacobian, sparseMarix.jacobian_size_, cudaMemcpyDeviceToHost);
        cudaFree(d_jacobian);
