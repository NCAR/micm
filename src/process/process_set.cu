...
namespace micm
{
  namespace cuda
  {
    ...
    std::chrono::nanoseconds AddForcingTermsKernelDriver(
        const VectorMatrix<double, L>& rate_constants,
        ...
        size_t yields_size)
    {
      ...
      cudaMemcpy(d_rate_constants, rate_constants.AsVector().data(), sizeof(double) * (n_grids * n_reactions), cudaMemcpyHostToDevice);
